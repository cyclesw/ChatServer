...
void ProcessRpcRequest(const std::string &request_body, const std::string &service_name, const std::string &log_prefix,
                       const std::shared_ptr<ServiceManager> &channels, StubMethod method,
      ...
      _http_server(std::make_shared<httplib::Server>())
{
    ...
    // (httplib::Server::Handler)std::bind(&GatewayServer::GetPhoneVerifyCode         , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(USERNAME_REGISTER      ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::UserRegister               , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(USERNAME_LOGIN         ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::UserLogin                  , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(PHONE_REGISTER         ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::PhoneRegister              , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(PHONE_LOGIN            ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::PhoneLogin                 , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(GET_USERINFO           ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::GetUserInfo                , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(SET_USER_AVATAR        ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::SetUserAvatar              , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(SET_USER_NICKNAME      ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::SetUserNickname            , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(SET_USER_DESC          ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::SetUserDescription         , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(SET_USER_PHONE         ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::SetUserPhoneNumber         , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(FRIEND_GET_LIST        ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::GetFriendList              , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(FRIEND_APPLY           ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::FriendAdd                  , this, std::placeholders::_1,
    // std::placeholders::_2));
    // // _http_server->Post(FRIEND_APPLY_PROCESS   ,_
    // (httplib::Server::Handler)std::bind(&GatewayServer::FriendAddProcess           , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(FRIEND_REMOVE          ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::FriendRemove               , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(FRIEND_SEARCH          ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::FriendSearch               , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(FRIEND_GET_PENDING_EV  ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::GetPendingFriendEventList  , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(CSS_GET_LIST           ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::GetChatSessionList         , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(CSS_CREATE             ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::ChatSessionCreate          , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(CSS_GET_MEMBER         ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::GetChatSessionMember       , this, std::placeholders::_1,
    // std::placeholders::_2));
    // // _http_server->Post(MSG_GET_RANGE          , (httplib::Server::Handler)std::bind(&GatewayServer::GetHistoryMsg
    // , this, std::placeholders::_1, std::placeholders::_2));
    // // _http_server->Post(MSG_GET_RECENT         , (httplib::Server::Handler)std::bind(&GatewayServer::GetRecentMsg
    // , this, std::placeholders::_1, std::placeholders::_2));
    // // _http_server->Post(MSG_KEY_SEARCH         , (httplib::Server::Handler)std::bind(&GatewayServer::MsgSearch ,
    // this, std::placeholders::_1, std::placeholders::_2)); _http_server->Post(NEW_MESSAGE            ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::NewMessage                 , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(FILE_GET_SINGLE        ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::GetSingleFile              , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(FILE_GET_MULTI         ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::GetMultiFile               , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(FILE_PUT_SINGLE        ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::PutSingleFile              , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(FILE_PUT_MULTI         ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::PutMultiFile               , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_server->Post(SPEECH_RECOGNITION     ,
    // (httplib::Server::Handler)std::bind(&GatewayServer::SpeechRecognition          , this, std::placeholders::_1,
    // std::placeholders::_2)); _http_thread = std::thread([this, http_port](){
...
void GatewayServer::KeepAlive(server_t::connection_ptr conn)
{
    ...
    _ws_server.set_timer(60000, [this, conn](const websocketpp::lib::error_code &ec) {
        if (ec)
        {
...
void GatewayServer::PhoneLogin(const httplib::Request &request, httplib::Response &response)
{
    ...
    auto err_response = [&phone_request, &phone_response, &response](const std::string &errmsg) -> void {
        phone_response.set_success(false);
        phone_response.set_errmsg(errmsg);
...
void GatewayServer::FriendAdd(const httplib::Request &request, httplib::Response &response)
{
    ...
    auto err_response = [&friend_request, &friend_response, &response](const std::string &errmsg) -> void {
        ...
        friend_response.set_errmsg(errmsg);
