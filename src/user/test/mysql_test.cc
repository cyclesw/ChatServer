...
void update_by_nickname(im::UserTable &user_tb) {
    auto user = user_tb.SelectByNickname("uid2");
    user->nickname("昵称2");
    user_tb.Update(user);
}
...
void select_users(im::UserTable &user_tb) {
    ...
    auto res = user_tb.SelectMultiUsers(id_list);
    for (auto user : res) {
        LOG_INFO(user.nickname());
    }
}
...
{    
    ...
    auto db = im::ODBFactory::create("test", "123456", "localhost", "test_db", "utf8", 0, 1);
