...
void Callback(brpc::Controller* cntl, ::ex::EchoResponse* response)
{
    std::unique_ptr<brpc::Controller> cntl_guard(cntl);
    std::unique_ptr<ex::EchoResponse> resp_guard(response);
...
int main(int argc, char** argv)
{
    ...
    ex::EchoService_Stub stub(&channel);
    //3. 进行Rpc调用/
    ex::EchoRequest req;
    req.set_message("你好~比特~！");

    brpc::Controller *cntl = new brpc::Controller();
    ex::EchoResponse *rsp = new ex::EchoResponse();
