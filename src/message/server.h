...
namespace im
{
    ...
    public:
        MessageServiceImpl(const std::shared_ptr<elasticlient::Client> &es_client,
                           ...
                           const std::shared_ptr<ServiceManager> &channel_manager, const std::string &file_service_name,
                           const std::string &user_service_name);

        ~MessageServiceImpl();

        void GetHistoryMessage(::google::protobuf::RpcController *controller,
                               const ::im::GetHistoryMessageRequest *request, ::im::GetHistoryMessageResponse *response,
                               ::google::protobuf::Closure *done) override;

        void GetRecentMessage(::google::protobuf::RpcController *controller,
                              const ::im::GetRecentMessageRequest *request, ::im::GetRecentMessageResponse *response,
                              ::google::protobuf::Closure *done) override;

        void MessageSearch(::google::protobuf::RpcController *controller, const ::im::MessageSearchRequest *request,
                           ::im::MessageSearchResponse *response, ::google::protobuf::Closure *done) override;

        void OnMessage(const char *body, size_t size);

    private:
        bool GetUser(const std::string& rid, const std::unordered_set<std::string>& user_id_lists, std::unordered_map<std::string, UserInfo>& user_lists);

        bool GetFile(const std::string& rid, const std::unordered_set<std::string>& file_id_lists, std::unordered_map<std::string, std::string>& file_data_lists);

        bool PutFile(const std::string& filename, const std::string& body, const int64_t fsize, std::string& file_id);
    ...
    public:
        ...
        MessageServer(const MQClient::Ptr &mq_client, const Discovery::Ptr service_discoverer,
                      const Register::Ptr &reg_client, const std::shared_ptr<elasticlient::Client> &es_client,
                      const std::shared_ptr<odb::core::database> &mysql_client,
